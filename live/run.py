...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Live verification test runner.",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    parser.add_argument(
        "--exact",
        action="store_true",
        help="use supplied names as exact matches",
    )
    parser.add_argument(
        "--skip-tests",
        metavar="PATTERNS",
        help="skip tests matching a comma-separated list of patterns",
    )
    parser.add_argument(
        "patterns",
        nargs="*",
        default=[],
        help="only run tests whose names match these patterns",
    )
...
def do_down(*, topology: TestTopology | None = None) -> None:
...
def do_run(
    *,
    ...
    skip_patterns = [pattern for pattern in (skip_tests or "").split(",") if pattern]
    ...
    for root, dirs, files in os.walk(tests_dir):
        ...
        relative_name = os.path.relpath(root, tests_dir)
        if patterns and not any(
            pattern in relative_name
            if not exact
            else pattern in (relative_name, test_name)
